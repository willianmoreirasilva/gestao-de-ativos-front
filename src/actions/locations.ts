...
// 🌟 1. Ajustado o tipo para aceitar o objeto simplificado com o id gerado
type ActionState = {
  error: string;
...
export async function upsertLocationAction(
  ...
  try {
    ...
    // 🌟 2. Captura limpa do ID conforme o padrão devolvido pela sua API
    const createdOrUpdatedId = response.data?.data?.id || id || "";

    revalidatePath("/infra/locations");

    // 🌟 3. Retorno idêntico e padronizado ao de departamentos
    return {
      error: "",
      fieldErrors: {},
      data: {

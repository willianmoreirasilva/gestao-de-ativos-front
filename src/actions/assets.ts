 ...
 */
export async function updateAssetAction(id: string, payload: UpdateAssetInput): Promise<MutationResponse> {
    try {
        const api = await getServerApi();
        
        // Dispara o PATCH mapeado pelo seu backend
       ...
       return {};
        ...
        return {
            error: error.response?.data?.error || "Não foi possível salvar o IP informado.",
        };
    }
}

// 🛠️ Troca rápida de IP no modal (Apenas IP)
export async function updateAssetOnlyIpAction(id: string,newIpId: UpdateAssetInput["newIpId"]): Promise<MutationResponse> {
    try {
        const api = await getServerApi();
        ...
        return {
            error: error.response?.data?.error || "Não foi possível vincular o IP informado.",
        };
    }
}
...
export async function findIpByAddressAction(
    address: string, 
    expectedVlanType: "GENERAL_DATA" | "CAMERA_VLAN" | "SWITCH_MGMT" | "WIFI_MGMT"
) {
    try {
        const api = await getServerApi();
        ...
        if (ipList.length === 0) {
            return { error: "Este endereço IP não está cadastrado na infraestrutura." };
        }
        ...
        if (exactIpMatch.status !== "AVAILABLE") {
            return { error: `Este endereço IP já está em uso (${exactIpMatch.status}) por outro ativo.` };
        }
        // Se passou em todas as regras, retorna o UUID correto
        return { data: exactIpMatch };
    } catch (error: any) {
        return { error: error.response?.data?.error || "Erro ao consultar regras do IP." };
    }
}
...
export function ComboboxCreateNew({
  ...
  // Dentro do seu ComboboxCreateNew:
  // Adicione essa verificação antes do .find para evitar que UUIDs malformados quebrem a tela
  ...
  const handleCreationSuccess = (newId: string) => {
    // Garante que estamos passando apenas a string limpa do UUID
    const idLimpo = String(newId).trim();

    console.log(
      "📥 Combobox recebeu o ID do modal e aplicando no Form:",
      idLimpo,
    );

    // Altera o valor no React Hook Form pai primeiro
    onChange(idLimpo);

    // Fecha os componentes visuais de forma segura no próximo ciclo de renderização
    setTimeout(() => {
      setOpenModal(false);
    ...
    <>
      <Popover open={openPopover} onOpenChange={setOpenPopover}>
        <PopoverTrigger asChild>
          <Button
            type="button"
            variant="outline" // Alterado para outline para manter o visual padrão de combobox com borda se preferir
            onClick={(e) => {
              e.stopPropagation();
              setOpenPopover(!openPopover); // 🌟 CORRETO: Abre/Fecha a listagem, não o modal!
            }}
            role="combobox"
        ...
        <PopoverContent
          className="w-full p-0 popover-content-width-same-as-trigger"
          align="start"
        >
          <Command>
      ...
      <Dialog open={openModal} onOpenChange={setOpenModal}>
        <DialogContent className="sm:max-w-[480px]">
          ...
          <div className="pt-2">{createForm(handleCreationSuccess)}</div>
        </DialogContent>
      </Dialog>
    </>
  );
